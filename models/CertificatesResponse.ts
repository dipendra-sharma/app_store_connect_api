
/**
 * App Store Connect API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 3.7.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { PagingInformation } from './PagingInformation';
import {
    PagingInformationFromJSON,
    PagingInformationFromJSONTyped,
    PagingInformationToJSON,
    PagingInformationToJSONTyped,
} from './PagingInformation';
import type { PagedDocumentLinks } from './PagedDocumentLinks';
import {
    PagedDocumentLinksFromJSON,
    PagedDocumentLinksFromJSONTyped,
    PagedDocumentLinksToJSON,
    PagedDocumentLinksToJSONTyped,
} from './PagedDocumentLinks';
import type { Certificate } from './Certificate';
import {
    CertificateFromJSON,
    CertificateFromJSONTyped,
    CertificateToJSON,
    CertificateToJSONTyped,
} from './Certificate';

/**
 * 
 * @export
 * @interface CertificatesResponse
 */
export interface CertificatesResponse {
    /**
     * 
     * @type {Array<Certificate>}
     * @memberof CertificatesResponse
     */
    data: Array<Certificate>;
    /**
     * 
     * @type {PagedDocumentLinks}
     * @memberof CertificatesResponse
     */
    links: PagedDocumentLinks;
    /**
     * 
     * @type {PagingInformation}
     * @memberof CertificatesResponse
     */
    meta?: PagingInformation;
}

/**
 * Check if a given object implements the CertificatesResponse interface.
 */
export function instanceOfCertificatesResponse(value: object): value is CertificatesResponse {
    if (!('data' in value) || value['data'] === undefined) return false;
    if (!('links' in value) || value['links'] === undefined) return false;
    return true;
}

export function CertificatesResponseFromJSON(json: any): CertificatesResponse {
    return CertificatesResponseFromJSONTyped(json, false);
}

export function CertificatesResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): CertificatesResponse {
    if (json == null) {
        return json;
    }
    return {
        
        'data': ((json['data'] as Array<any>).map(CertificateFromJSON)),
        'links': PagedDocumentLinksFromJSON(json['links']),
        'meta': json['meta'] == null ? undefined : PagingInformationFromJSON(json['meta']),
    };
}

export function CertificatesResponseToJSON(json: any): CertificatesResponse {
    return CertificatesResponseToJSONTyped(json, false);
}

export function CertificatesResponseToJSONTyped(value?: CertificatesResponse | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'data': ((value['data'] as Array<any>).map(CertificateToJSON)),
        'links': PagedDocumentLinksToJSON(value['links']),
        'meta': PagingInformationToJSON(value['meta']),
    };
}

