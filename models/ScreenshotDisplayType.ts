...
export function instanceOfScreenshotDisplayType(value: any): boolean {
    for (const key in ScreenshotDisplayType) {
        if (Object.prototype.hasOwnProperty.call(ScreenshotDisplayType, key)) {
            if (ScreenshotDisplayType[key as keyof typeof ScreenshotDisplayType] === value) {
                return true;
            }
        }
    }
...
export function ScreenshotDisplayTypeToJSONTyped(value: any, ignoreDiscriminator: boolean): ScreenshotDisplayType {
    return value as ScreenshotDisplayType;
}
